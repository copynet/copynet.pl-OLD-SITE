<?php
error_reporting(0);//comment out to see PHP Errors
	/**
...
define('IMG_EXT','[\'jpeg\', \'jpg\', \'gif\', \'png\', \'bmp\', \'pdf\']');// Used for javascript validation
define('DATE_FORMAT','d/m/Y H:i');
define('FILE_CHAR_TO_SHOW','50');//filename will be truncated to 25chars if it's longer

$ezfm_script_url = "http://".$_SERVER['SERVER_NAME'].$_SERVER['SCRIPT_NAME'];
$ezfm_script_path = $_SERVER['SCRIPT_FILENAME'];
$under_root_dir = "tinymce"; /*first directory wich is under root where you have ezfilemanager;
																	ex:	 root dir of you website: /home/username/public_html/
																		ezfilemanager install path: /home/username/public_html/tinymce/tiny_mce/plugins/ezfilemanager/
																								under_root_dir: modules			*/
$s_pos = stripos($ezfm_script_url, $under_root_dir);
if($s_pos === false) die ("Error" . " File: " . __FILE__ . " on line: " . __LINE__);
define('WN_URL', substr($ezfm_script_url, 0, $s_pos));//your domain url
//define('WN_URL', 'http://www.yourdomain.com/');//your domain url
$s_pos = stripos($ezfm_script_path, $under_root_dir);
if($s_pos === false) die ("Error" . " File: " . __FILE__ . " on line: " . __LINE__);
define('WN_PATH', substr($ezfm_script_path, 0, $s_pos));//domain root absolute path
//define('WN_PATH','/home/username/public_html/');//domain root absolute path

define("IMG_PATH",WN_PATH."../images/");//Image dir absolute path
define('IMG_URL',WN_URL.'../images/');//Image dir url
define('FILE_PATH',WN_PATH.'../docs/');//File dir absolute path
define('FILE_URL',WN_URL.'../docs/');//File dir url
define('MEDIA_PATH',WN_PATH.'../images/');//Media dir absolute path
define('MEDIA_URL',WN_URL.'../images/');//Media dir url
	/**
	 * ezfilemanager array
	 ...
	 */
$ezfilemanager = array();
$ezfilemanager['maxsize']['image'] = 4096000; // Image file maximum size 61440=60KB
$ezfilemanager['maxsize']['media'] = 4096000; // Media file maximum size 2048000=2M
$ezfilemanager['maxsize']['file']  = 4096000; // Other file maximum size 1228800=1.2M
	/**
	 * Add Permitted file extensions  to ezfilemanager array
	 * for image, media and docs/other types
	 */
$ezfilemanager['filetype']['image'] = array('image/pjpeg', 'image/jpeg', 'image/gif', 'image/x-png', 'image/png', 'image/bmp','pdf');
$ezfilemanager['filetype']['media'] =  array('swf','mp3','mov','avi','mpg','qt','mp4','ram');
...
$ezfilemanager_mime_types = array(
	...
	array(array("mov", "mpg", "avi", "asf", "mpeg", "wmv","mp4","qt","ram"), "movie.gif", "movie", false),
...
$ezfilemanager['url']['file']  = FILE_URL;
	 ...
	 * Do not modify
	 */
if(!isset($_GET['sa'])) $_GET['sa'] = false;
if(!isset($_GET['type']) || !in_array($_GET['type'], array('image', 'media', 'file'))) $_GET['type'] = '';
if(!isset($_GET['thedir'])) $_GET['thedir'] = "";
